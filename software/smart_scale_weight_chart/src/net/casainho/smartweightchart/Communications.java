...
public class Communications {
	...
	//
	//
	String[] receiveMessage(byte[] messageBytes, int messageBytesIndex,
			int messageSize) {
		...
		for (; messageBytesIndex < messageSize; messageBytesIndex++) {
			...
			case 2: // get all data byte to Vector
				...
				if (mBytesCounterReceiveMesssage < mLenReceiveMessage) {
					mReceivedMessageVector
							.add((int) messageBytes[messageBytesIndex]);
					mBytesCounterReceiveMesssage++;
					...
					if (mBytesCounterReceiveMesssage == mLenReceiveMessage) {
						...
						mReceivedMessageVector
								.removeElementAt((mReceivedMessageVector.size() - 1));
						...
						if (mCRCReceiveMessage == (CRCReceiveMessage.calc(
								mReceivedMessageVector,
								(mLenReceiveMessage - 2)))) {// (CRCReceiveMessage.calc(mReceivedMessageVector,
																// (mLenReceiveMessage-2))))
																// {
							// CRC is ok
							mReceivedMessageVector.removeElementAt(0); // remove
																		// length
																		// byte

							byte[] byteArrayMessage = new byte[(mReceivedMessageVector
									.size() - 2)];
							for (int j = 2; j < (mLenReceiveMessage - 3); j++) {
								byteArrayMessage[j - 2] = (byte) ((mReceivedMessageVector
										.get(j)) & 0xff);
							}

							String s = new String(byteArrayMessage);
							returnMessage[0] = s;
							returnMessage[1] = Integer
									.toString(messageBytesIndex);
