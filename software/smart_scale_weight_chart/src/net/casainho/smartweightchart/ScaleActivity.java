		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_READ:
				...
				if (bundleObj.containsKey("weight")) {
					...
					mScaleTextView.setText(String.format("%1.1f",
							(((float) (weight)) / 10)) + " KG");
		...
		case RESULT_FIND_DEVICE:
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				String address = data.getExtras().getString(
						FindDeviceActivity.EXTRA_DEVICE_ADDRESS);

				// Read BT initial state
				mBTInitState = data.getExtras().getInt(
						FindDeviceActivity.EXTRA_BT_INITIAL_STATE);

				// Get the BluetoothDevice object
				BluetoothDevice device = mBluetoothAdapter
						.getRemoteDevice(address);

				// Initialize the BluetoothCommService to perform bluetooth
				// connections
			...
			// For some reason FindDeviceActivity didn't return a bluetooth
			// device address
			else {
				Toast.makeText(this, "not bluetooth device", Toast.LENGTH_SHORT)
						.show();
				finish();
			}
			break;
